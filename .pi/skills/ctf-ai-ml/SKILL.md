...
**AI/ML 分析混合执行**：模型分析在 Docker，计算在本机。

| 命令 | 执行环境 | 原因 |
|------|---------|------|
| `python3` (torch/transformers) | Docker | 需要 GPU/大内存 |
| `python3` (numpy/sklearn) | 本机 | 轻量计算 |
| 模型文件分析 | Docker | 二进制分析 |

```
# 模型分析 - Docker
sandbox_init(path="model.bin")
sandbox_exec(cmd="python3 -c 'import torch; m=torch.load(\"/workspace/sample\"); print(m)'")
