...
**密码学分析大部分在本机执行**，仅文件相关操作在 Docker。

| 命令 | 执行环境 | 原因 |
|------|---------|------|
| `python3` (pycryptodome/z3/sympy) | 本机 | 纯计算 |
| `sage` (SageMath) | 本机 | 数学库 |
| `hashcat`, `john` | 本机/Docker | 密码破解 |
| `RsaCtfTool.py` | 本机 | RSA 自动攻击 |
| `openssl` | 本机 | 证书操作 |
| 文件操作 (`file`, `strings`) | Docker | 如需分析二进制 |
...
# 如需分析加密二进制
sandbox_init(path="encrypted.bin")
sandbox_exec(cmd="xxd /workspace/sample | head -20")    ← Docker
