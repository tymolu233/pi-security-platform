...
**Web 测试全部在本机执行**（需要网络访问目标）。

| 命令 | 执行环境 | 原因 |
|------|---------|------|
| `curl`, `wget` | 本机 | HTTP 请求 |
| `nmap` | 本机 | 端口扫描 |
| `sqlmap` | 本机 | SQL 注入 |
| `python3` (requests) | 本机 | HTTP 脚本 |
| `dirb`, `gobuster` | 本机 | 目录爆破 |

```
curl -s http://target                                    ← 本机
sqlmap -u 'http://target?id=1' --batch                   ← 本机
nmap -sV target                                          ← 本机
python3 -c "import requests; ..."                        ← 本机
```
