...
license: MIT
compatibility: Requires filesystem-based agent (Claude Code or similar) with bash, Python 3, and internet access for tool installation. On Windows, run Linux-only tooling through WSL.
...
**取证分析混合执行**：文件分析在 Docker，网络/在线在本机。

| 命令 | 执行环境 | 原因 |
|------|---------|------|
| `binwalk`, `foremost` | Docker | 固件/磁盘分析 |
| `steghide`, `exiftool` | Docker | 隐写/元数据 |
| `volatility3` | Docker | 内存取证 |
| `strings`, `xxd`, `file` | Docker | 文件分析 |
| `tshark`, `tcpdump` | 本机/Docker | 网络包分析 |
| `curl` (在线工具) | 本机 | 在线查询 |

```
# 文件分析 - Docker
sandbox_init(path="evidence.img")
sandbox_exec(cmd="binwalk -e /workspace/sample")
sandbox_exec(cmd="strings /workspace/sample | grep flag")
...
**Windows (use WSL when a step requires Linux tools):**
```bash
wsl -e bash -lc "<linux-command>"
```
