...
license: MIT
compatibility: Requires filesystem-based agent (Claude Code or similar) with bash, Python 3, and internet access for OSINT lookups. On Windows, run Linux-only tooling through WSL.
...
**OSINT 全部在本机执行**（需要网络访问）。

| 命令 | 执行环境 | 原因 |
|------|---------|------|
| `whois` | 本机 | 域名查询 |
| `dig`, `nslookup` | 本机 | DNS 查询 |
| `curl`, `wget` | 本机 | HTTP 请求 |
| `python3` (API 调用) | 本机 | 在线查询 |

```
whois target.com                                        ← 本机
dig target.com ANY +short                               ← 本机
curl -s 'https://crt.sh/?q=%25.target.com&output=json'  ← 本机
```
