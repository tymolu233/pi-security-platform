...
**杂项混合执行**：编码/计算在本机，沙箱逃逸在 Docker。

| 命令 | 执行环境 | 原因 |
|------|---------|------|
| `python3` (编码/解码) | 本机 | 纯计算 |
| `z3`, `sympy` | 本机 | 约束求解 |
| `curl`, `wget` | 本机 | 网络请求 |
| `base64`, `xxd` | 本机 | 编码转换 |
| pyjail/bashjail 分析 | Docker | 沙箱逃逸测试 |
| `qemu` (模拟) | Docker | 架构模拟 |
...
# Z3 求解 - 本机
python3 -c "from z3 import *; ..."

# 沙箱逃逸测试 - Docker
sandbox_init(path="jail")
sandbox_exec(cmd="python3 -c 'print(().__class__.__bases__)'")
